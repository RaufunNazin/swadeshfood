...
const FreeDeliveryBar = ({ subtotal }) => {
  ...
  if (subtotal === 0) return null; // Hide if cart is empty
  ...
  return (
    <div className="w-full bg-green-50 dark:bg-green-900/10 border-b border-green-100 dark:border-green-900/30 px-4 py-3 transition-colors duration-300">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-center gap-2 md:gap-6">
        {/* Text Area */}
        <div
          className={`flex items-center gap-2 text-sm md:text-base font-bold tracking-wide transition-colors ${isUnlocked ? "text-green-600 dark:text-green-400" : "text-neutral-700 dark:text-neutral-300"}`}
        >
          {isUnlocked ? (
        ...
        {/* Progress Bar (Hidden on Mobile if unlocked to save space, visible otherwise) */}
        {!isUnlocked && (
